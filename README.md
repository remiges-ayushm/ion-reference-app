...
5. Upload the two files that rarely change and aren't generated by anything
   in this repo:
   ```sh
   gsutil cp dedi.index.json gs://$(terraform output -raw dedi_static_bucket)/.well-known/dedi.index.json
   gsutil cp ion-scratch-registry.json gs://$(terraform output -raw dedi_static_bucket)/dedi/ion-scratch-registry.json
   ```
